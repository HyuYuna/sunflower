...
<script>
	$(function(){
		$('tbody[id^=resultTbody_]').each(function(){
			...
			getUserAnswer(qesitmSn, 'Y');
		});
		$('ol[id^=resultOl_]').each(function(){
			var qesitmSn = this.id.replace('resultOl_', '');
			getUserAnswer(qesitmSn, 'N');
		});
	});

	function getUserAnswer(qesitmSn, objectiveYn) {
		...
		$.post(
			"/bos/qestnar/userAnswer/list.json",
			{qesitmSn: qesitmSn, pageUnit: pageUnit, pageIndex: pageIndex, objectiveYn: objectiveYn},
			function(data) {
				...
				$.each(r, function(i, item) {
					...
					$('#surveyRsult_'+item.qesitmSn).show();
...
<div class="view">
	<p class="subject">${result.qustnrSj}</p>
	<dl>
		<dt>설문대상</dt>
		<dd>
...
<div class="btnSet">
	<a href="downloadExcel.do?qustnrSn=${param.qustnrSn}&viewType=CONTBODY" class="b-down">설문조사결과 엑셀다운</a>
	<a href="downloadUserAnswerExcel.do?qustnrSn=${param.qustnrSn}&viewType=CONTBODY" class="b-down">설문조사참여현황 엑셀다운</a>
	<a href="/bos/qestnar/qustnr/list.do?${pageQueryString}" class="b-list">목록</a>
</div>

<h2>설문결과</h2>
<c:forEach var="qesitm" items="${qesitmList}" varStatus="status">
	<div class="surveyResultListItem">
	<c:if test="${qesitm.qesitmTyCd eq 1 or qesitm.qesitmTyCd eq 2}">
		<div class="bdList">
			...
			<div class="surveyRsult"><p>${status.count}. ${qesitm.qesitmSj }</p></div>
			<table class="table table-survey table-bordered">
				<thead>
					<tr>
		...
		<div class="surveyRsult" id="surveyRsult_${qesitm.qesitmSn}" style='display:none;'>
			<div class="sh">
				...
				<button type="button" onclick="getUserAnswer('${qesitm.qesitmSn}')" class="b-ok">이동</button>
				...
				<button type="button" onclick="getUserAnswer('${qesitm.qesitmSn}')" class="b-ok">적용</button>
			</div>
			<table class="table table-bordered">
				<caption>${status.count}번 답변 목록</caption>
				<thead>
					<tr>
				...
				<tbody id="resultTbody_${qesitm.qesitmSn}">
				</tbody>
			</table>
		</div>
	</c:if>
