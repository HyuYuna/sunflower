...
<div>
	<table class="table table-striped table-hover">
		...
		<thead>
			<tr>
				<th style="width:13%"		scope="col">번호</th>
				<th style="width:13%"		scope="col">이름</th>
				<th style="width:14%"		scope="col">아이디</th>
				<th style="width:20%"		scope="col">회원가입일</th>
				<th	style="width:20%"		scope="col">마지막로그인 일시</th>
				<th style="width:20%"		scope="col">휴면일시</th>
			</tr>
		</thead>
		<tbody>
		<c:forEach var="result" items="${resultList}" varStatus="status">
			...
			<tr>
				...
				<td>${result.userNm}</td>
				<td>${fn:substring(result.userId,0,3)}${fn:substring('*************************',0,fn:length(result.userId)-3)}</td>
				<td>${fn:substring(result.registDt,0,19)}</td>
				<td>${fn:substring(result.lastLoginDt,0,19)}</td>
				<td>${fn:substring(result.drmncyDt,0,19)}</td>
			</tr>
