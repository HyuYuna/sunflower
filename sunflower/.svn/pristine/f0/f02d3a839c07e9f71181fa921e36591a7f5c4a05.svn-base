	...
	<table class="table table-striped table-hover">
		...
		<thead>
			<tr>
				...
				<th style="width:7%"		scope="col">이름</th>
				<th style="width:7%"		scope="col">아이디</th>
				<th style="width:8%"		scope="col">IP</th>
				<th							scope="col">메뉴</th>
				<th style="width:10%"		scope="col">행동</th>
				<th							scope="col">URL</th>
				<th style="width:12%"		scope="col">일시</th>
				<th style="width:12%"		scope="col">로그인일시</th>
			</tr>
		</thead>
		<tbody>
		<c:forEach var="result" items="${resultList}" varStatus="status">
			...
			<tr>
				...
				<td class="tal">${result.relateMenuNmList}</td>
				<td>${result.ghvrSumry}</td>
				<td class="tal">${result.conectUrlad}</td>
				<td>${fn:substring(result.registDt,0,19)}</td>
				<td>${fn:substring(result.userLoginDt,0,19)}</td>
			</tr>
