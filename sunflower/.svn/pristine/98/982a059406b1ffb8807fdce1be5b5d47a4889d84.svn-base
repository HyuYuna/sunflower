...
public class Seed256Util {
	...
	private static int KEY_SIZE = 48;
	...
	public static String Encrypt (String password) {
		if(Seed256Util.isEmpty (password)) {
			return null;
		}
		if(password.length () == 1) {
			...
			for (byte hash : result) {
				resultHash += Seed256Util.addZero (Integer.toHexString (0xff & hash), 2);
			}
			return resultHash;
		}
	}
...
/*	public static void main(String args[]) {
		...
		str = Encrypt(str);
		log.error("## encrypt : "+str);
		str = Decrypt (str);
		log.error("## decrypt : "+str);
	}*/
}
