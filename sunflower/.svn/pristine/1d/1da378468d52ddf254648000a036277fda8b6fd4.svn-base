...
function fncSelectPrhibtList(pageNo){
		...
		document.listForm.action = "/bos/sys/prhibtWrdFlter/list.do?menuNo=${param.menuNo}";
		document.listForm.submit();
}

// 금지단어 등록버튼
function fncAddPrhibtInsert() {
		location.replace("/bos/sys/prhibtWrdFlter/forInsert.do?menuNo=${param.menuNo}");
}
...
function fncPrhibtDeleteList() {
		if(fncManageChecked()) {
				if(confirm("삭제하시겠습니까?")) {
						document.listForm.action = "/bos/sys/prhibtWrdFlter/delete.do?menuNo=${param.menuNo}";
						document.listForm.submit();
				}
		}
}
...
function fncAddPrhibtView(prhibt) {
	...
	document.listForm.action = "/bos/sys/prhibtWrdFlter/forUpdate.do?menuNo=${param.menuNo}";
		document.listForm.submit();
}
...
$(function() {
	...
	<form name="listForm" id="listForm" action="/bos/sys/prhibtWrdFlter/list.do" method="get">
		...
		<div class="sh">
			...
			<%-- 	<input type="text" id="sdate" name="sdate" value="${param.sdate}" size="10" /> ~
				...
				<a href="/bos/sys/prhibtWrdFlter/list.do?menuNo=${param.menuNo}" class="b-reset">초기화</a>
			</fieldset>
		</div>
