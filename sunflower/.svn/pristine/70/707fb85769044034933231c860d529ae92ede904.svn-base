...
function fnGetCdDetail(id) {
	if (id != 0) {
		$.ajax({ url : "/bos/cmmnCd/cmmnCdDetail/listJson.json?codegCode="+ vcodegCode + "&codeCode="+id, dataType : "json", async: false }).done(function (resdata) {
			for (key in resdata) {
				if (key != "useAt") $('#'+key).val(resdata[key]);
			}
			$("#dpTd").text(resdata.dp);
			$("#codeForm").find('input:radio[name=useAt]').filter(':input[value='+resdata.useAt+']').prop("checked", true);
		...
		$("#codeForm").find('input:radio[name=useAt]:input[value=Y]').prop("checked", true);
	}
...
$(function () {
	...
	.jstree({
		'core' : {
			'data' : function (node, cb) {
				var t_url = node.id != "#" ? "/bos/cmmnCd/cmmnCdDetail/listJson.json?codegCode=${paramVO.codegCode}&upperCd="+node.id : "/bos/cmmnCd/cmmnCdDetail/listJson.json?codegCode=${paramVO.codegCode}";
	            $.ajax({ url :t_url, dataType : "json" }).done(function (data) {

	            	var rootList = [];
	            	var jsonList = [];
	            	var root = {'id' : '0', 'text' : "${cdCtgryVO.codegName}(Root)",'icon' : "jstree-folder",'children':true, 'data' : [{'useAt' : "Y"}], 'state' : { 'opened' : true, 'selected' : true }};
	...
	.on("click", ".jstree-anchor", function(e) {
	   var node = $("#cdTree").jstree(true).get_node($(this));
	   if (node.id == "0") return;
	   var goUrl = "/bos/cmmnCd/cmmnCdDetail/listPopup.do?viewType=BODY&cdId=" + node.data.cdId + "&cd=" + node.id;
	...
	*/
	.on('delete_node.jstree', function (e, data) {
		...
		if (!confirm("코드를 삭제 하시겠습니까? (삭제하시면 복구가 불가능합니다.)")) {
			...
			$.ajax({ url : "/bos/cmmnCd/cmmnCdDetail/delete.json", data : { 'codegCode' : vcodegCode, 'codeCode' : codeCode }, dataType : "json", async: false }).done(function (resdata) {
	...
	.on('create_node.jstree', function (e, data) {
		$.ajax({ url : "/bos/cmmnCd/cmmnCdDetail/insert.json", data : {'cdId' : vCdId, 'cdNm' : escape(encodeURIComponent("신규메뉴")), 'upperMenuNo' : data.node.parent, 'useAt' : "N", 'popupAt' : "N" }, dataType : "json", async: false }).done(function (resdata) {
	...
	*/
	...
	.on('rename_node.jstree', function (e, data) {
		...
		$.ajax({ url : "/bos/cmmnCd/cmmnCdDetail/updateCdNm.json", data : { 'cdId' : vCdId, 'cd' : data.node.id, 'cdNm' : escape(encodeURIComponent(data.node.text)), 'upperCd' : data.node.parent, 'useAt' : useAtVal }, dataType : "json", async: false }).done(function (resdata) {
			data.instance.set_id(data.node, resdata.cMenuNo);
	...
	*/
	.on('move_node.jstree', function (e, data) {
		if (!confirm((data.node.parents.length-1) + "depth "+  "["+data.node.data.codeName +"] 코드를 이동하시겠습니까?")) {
			data.instance.refresh();
			return false;
		}
		...
		if (data.old_parent != data.parent) { //부모키가 틀릴경우
			$.ajax({ url : "/bos/cmmnCd/cmmnCdDetail/updateUpperCd.json", data : { 'codegCode' : vcodegCode, 'cd' : data.node.id, 'upperCd' : data.node.parent }, dataType : "json", async: false }).done(function (resdata) {
			...
			$.ajax({ url : "/bos/cmmnCd/cmmnCdDetail/updateMoveCd.json", data : { 'codegCode' : vcodegCode, 'cd' : data.node.id, 'upperCd' : data.node.parent, 'cdData' : cdData }, dataType : "json", async: false }).done(function (resdata) {
...
function updateMoveCd(gubun) {
	...
	if (gubun == "U") {
		...
		var cdData = [];
		$(instance.get_node(instance.get_parent(selected)).children).each(function(e, val) {
			if (cd != val) 	cdData.push(val);
		});
		if (gubun == "T") cdData.splice(0, 0, cd);
		else if (gubun == "B") cdData.push(cd);
	...
	$.ajax({ url : "/bos/cmmnCd/cmmnCdDetail/updateMoveCd.json", data : { 'codegCode' : vcodegCode ,  'cd' : cd, 'cdOrdr' : $("#cdOrdr").val(), 'cdData' : $("#cdData").val(), 'chgCd' : chgCd, 'gubun' : gubun }, dataType : "json", async: false }).done(function (resdata) {
...
document.title = "${cdCtgryVO.clCdNm}(${cdCtgryVO.codegCode}) 공통상세코드 관리";
</script>
...
<div class="row">
	...
	<div class="col-lg-6">
		...
		<div class="bdView afterDiv" id="updtView">
			<form  id="codeForm" name="codeForm" action="/bos/cmmnCd/cmmnCdDetail/update.do" method="post">
			...
			<input type="hidden" name="sortOrdr" value="${result.sortOrdr }" title="정렬순서"/>
			<table>
				...
				<tbody>
			 	<tr>
				    ...
				    <td>
				    	<input name="upperCd" id="upperCd" type="text" size="15" value="${not empty result.upperCd  ? result.upperCd : '0' }"  maxlength="15" readonly="readonly" title="상위코드" />
				    </td>
			  	</tr>
			  	<tr>
				    <th scope="row"><span class="req"><span>필수입력</span></span>코드</th>
				    <td nowrap>
				      <input name="cd" id="cd" type="text" size="15" value="${not empty result.cd ? result.cd  :  '0' }"  maxlength="15" <c:if test="${not empty result or empty param.cd}">readonly="readonly"</c:if> title="코드">
				    </td>
			  	</tr>
			  	<tr>
				    <th scope="row"><span class="req"><span>필수입력</span></span>코드명</th>
				    <td nowrap>
				      <input name="cdNm" id="cdNm" type="text" size="30" value="<util:out escapeXml='false'>${result.cdNm}</util:out>"  maxlength="30" itle="코드명">
				    </td>
			  	</tr>
				...
				<!--
			  	<tr>
				    ...
				    <td nowrap>
				      <input name="cdEngNm" id="cdEngNm" type="text" size="30" value="${result.cdEngNm}"  maxlength="30" />
				    </td>
			  	</tr>
			  	-->

			  	<tr>
				    <th scope="row">Depth level</th>
				    <td nowrap id="dpTd">
				      ${result.dp}
				    </td>
			  	</tr>
			  	...
			  	<tr>
				    ...
				    <td nowrap>
				      <textarea name="cdDc" id="cdDc" cols="25" rows="4" title="코드설명"><util:out escapeXml='false'>${result.cdDc }</util:out></textarea>
				    </td>
			  	</tr>
				...
				<!--
			  	<tr>
				    ...
				    <td nowrap>
				      <textarea name="cdEngDc" id="cdEngDc" cols="25" rows="4">${result.cdEngDc }</textarea>
				    </td>
			  	</tr>
			  	...
			  	<tr>
					...
					<td>
				   	<input type="text" name="sortOrdr" size="15" maxlength="15" value="${result.sortOrdr }" title="정렬순서"/>
					</td>
			 	</tr>
			 	-->
				...
				<tr>
					<th scope="row">옵션1</th>
					<td>
				      <input type="text" name="optn1" size="15" maxlength="15" value="${result.optn1 }" />
					</td>
			 	</tr>
				<tr>
					<th scope="row">옵션2</th>
					<td>
				      <input type="text" name="optn2" size="15" maxlength="15" value="${result.optn2 }" />
					</td>
				</tr>
				<tr>
					<th scope="row">옵션3</th>
					<td>
				      <input type="text" name="optn3" size="15" maxlength="15" value="${result.optn3 }" />
					</td>
			 	</tr>
				<tr>
					<th scope="row">옵션4</th>
					<td>
				      <input type="text" name="optn4" size="15" maxlength="15" value="${result.optn4 }" />
					</td>
			 	</tr>
				<tr>
					<th scope="row">옵션5</th>
					<td>
				      <input type="text" name="optn5" size="15" maxlength="15" value="${result.optn5 }" />
					</td>
			 	</tr>
			  	-->
			  	<tr>
				    ...
				    <td>
				      <label><input name="useAt" id="useAt" type="radio" value="Y" <c:if test="${result.useAt eq 'Y' }">checked="checked"</c:if> /> 사용 </label>
				      <label><input name="useAt" type="radio" value="N" <c:if test="${result.useAt eq 'N' }">checked="checked"</c:if> />미사용</label>
				    </td>
			  	</tr>
		...
		<div class="bdView" id="registView" style="display:none">
			<form  id="codeRegForm" name="codeRegForm" action="/bos/cmmnCd/cmmnCdDetail/insert.do" method="post">
			<input type="hidden" name="csrfToken" value="${csrfToken}"/>
			<input name="codegCode" type="hidden" value="${param.codegCode}" />
			<input name="viewType" type="hidden" value="BODY" />
			<table>
				...
				<tbody>
			  	<tr>
					...
					<td>
					<input name="upperCd" type="text" size="15" value="${not empty result.upperCd  ? result.upperCd : '0' }"  maxlength="15" readonly="readonly" title="상위코드">
					</td>
			  	</tr>
			  	<tr>
				    <th scope="row"><span class="req"><span>필수입력</span></span>코드</th>
				    <td nowrap>
				      <input name="cd" type="text" size="15" value=""  maxlength="15" title="코드" />
				    </td>
			  	</tr>
			  	<tr>
				    <th scope="row"><span class="req"><span>필수입력</span></span>코드명</th>
				    <td nowrap>
				      <input name="cdNm" type="text" size="30" value=""  maxlength="30" title="코드명" />
				    </td>
			  	</tr>
				<!--
			  	<tr>
				 ...
				 -->
			  	...
			  	<tr>
					<th scope="row"><span class="req"><span>필수입력</span></span>정렬순서</th>
					<td>
				      <input type="text" name="sortOrdr" size="15" maxlength="15" value="" title="정렬순서" />
					</td>
			 	</tr>
				-->
				<!--
				...
				<tr>
			  	...
			  	<tr>
				    <th scope="row"><span class="req"><span>필수입력</span></span>사용여부</th>
				    <td>
				    	<label><input name="useAt" type="radio" value="Y" checked="checked" /> 사용 </label>
				    	<label><input name="useAt" type="radio" value="N" />미사용 </label>
				    </td>
			  	</tr>
...
<div class="btnSet clear" id="updtBtnView">
	...
	<a href="javascript:void(0);" class="b-add" id="downCdRegistBtn" data-value="${empty param.cd ? '0' : param.cd }">하위코드등록</a>
...
<script>
	$("#topCdRegistBtn").on('click',function() {
		...
		var url ="/bos/cmmnCd/cmmnCdDetail/getSortOrdr.json";
		var params = {
			codegCode : $regFrm.find("input[name=codegCode]").val(),
			upperCd : $regFrm.find("input[name=upperCd]").val()
		};
		$.post(url,params,function(data) {
			if (data.resultCode == "success") {
				$regFrm.find("input[name=sortOrdr]").val(data.maxSortOrdr);
			}
		},"json");

		$regFrm.find("input[name=upperCd]").val(upperCd);
	...
	$("#downCdRegistBtn").on('click',function() {
		...
		var $frm = $("#codeForm");
		var $regFrm = $("#codeRegForm");
		var upperCd = $frm.find("input[name=cd]").val();
		$regFrm.find("input[name=upperCd]").val(upperCd);
		...
		$("#inputTitle").text("하위코드등록폼-("+$frm.find("input[name=cdNm]").val()+" : "+$frm.find("input[name=cd]").val()+")");
		return false;
	});
	...
	$("#backBtn").on('click',function() {
		...
		var $frm = $("#codeForm");
		var upperCd = $frm.find("input[name=upperCd]").val();
	...
	$("#saveBtn").on('click',function() {
		...
		else if ($regFrm.find("input[name=upperCd]").val() == "") {
			alert("상위코드를 입력해주세요.");
			$regFrm.find("input[name=upperCd]").focus();
			return false;
		}
		else if ($regFrm.find("input[name=cd]").val() == "") {
			alert("코드를 입력해주세요.");
			$regFrm.find("input[name=cd]").focus();
			return false;
		}
		else if ($regFrm.find("input[name=cdNm]").val() == "") {
			alert("코드명을 입력해주세요.");
			$regFrm.find("input[name=cdNm]").focus();
			return false;
		}
		...
		//$regFrm.submit();
		$.post("/bos/cmmnCd/cmmnCdDetail/insert.json",$regFrm.serialize(),function(data) {
			if (data.resultCode == "success") {
				alert(data.msg);
	...
	$("#updtBtn").on('click',function() {
		...
		else if ($frm.find("input[name=upperCd]").val() == "") {
			alert("상위코드를 입력해주세요.");
			$frm.find("input[name=upperCd]").focus();
			return false;
		}
		else if ($frm.find("input[name=cd]").val() == "") {
			alert("코드를 입력해주세요.");
			$frm.find("input[name=cd]").focus();
			return false;
		}
		else if ($frm.find("input[name=cdNm]").val() == "") {
			alert("코드명을 입력해주세요.");
			$frm.find("input[name=cdNm]").focus();
			return false;
		}
		...
		$frm.attr("action","/bos/cmmnCd/cmmnCdDetail/update.do");
		$frm.submit();
		return false;
	});
	...
	$("#deleteBtn").on('click',function() {
		...
		$frm.attr("action","/bos/cmmnCd/cmmnCdDetail/delete.do");
		$frm.submit();
	});
