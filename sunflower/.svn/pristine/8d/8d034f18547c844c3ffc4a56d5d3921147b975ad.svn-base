	...
	<table>
		...
		<thead>
			<tr>
				...
				<th scope="col" >제목</th>
				<th scope="col" style="width:85px;">첨부</th>
			...
			<tr>
				...
				<td>
					<c:if test="${not empty result.atchFileId and result.fileCount > 0}">
						<a href="/ucms/cmmn/file/fileDown.do?menuNo=${param.menuNo}&atchFileId=${result.atchFileId}&fileSn=${result.fileSn}">
							<img src="/static/commons/img/disk.gif" alt="${result.orignlFileNm}"/>
						</a>
					</c:if>
				</td>
				<td>${result.ntcrNm}</td>
