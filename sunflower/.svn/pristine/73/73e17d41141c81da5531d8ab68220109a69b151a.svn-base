...
// 한글 입력 제한       $('.noKorean input').css('ime-mode','disabled');
    ...
    function checkForm() {
        ...
        if($("#caseSeq").val() == "0"){
			$("#formCase").attr("action", "/bos/instance/case/insert.do");
		} else {
			$("#formCase").attr("action", "/bos/instance/case/update.do");
		}
        form.submit();
    }
...
<form id="formCase" name="formCase" method="post" enctype="multipart/form-data" action="${action}">
	<input type="hidden" name="caseSeq" id="caseSeq" value="${empty result.caseSeq ? 0 : result.caseSeq }" /> 
	<input type="hidden" id="caseAttkrSeq" name="caseAttkrSeq" value="${result.caseAttkrSeq}" /> 
	...
	<input type="hidden" id="caseNumber" name="caseNumber" value="${result.caseNumber}" /> <!-- 전산관리번호 자동생성 -->
	<input type="hidden" id="memberId" name="memberId" value="${empty result ? userVO.userId : result.memberId}" /> <!-- 접수담당자 없음 -->
	<input type="hidden" id="caseHelp2" name="caseHelp2" value="" /> <!-- 도움 요청 내용(중복) -->
	<input type="hidden" id="attkrState2" name="attkrState2" value="" /> <!-- 가해시 특이사항 -->
	<input type="hidden" id="dmgState2" name="dmgState2" value="" /> <!-- 피해시 특이사항 -->
	<input type="hidden" id="caseRel2" name="caseRel2" value="" /> <!-- 참조사례번호 -->
	<input type="hidden" id="attkr" name="attkr" value="${act eq 'insert' ? 'Y' : '' }" /> 
	<input type="hidden" id="act" name="act" value="${act }" /> 
	<input type="hidden" id="tmpr" name="tmpr" value="" /> 
	<input type="hidden" name="today" value="${today}" /> 
	
	<input type="hidden" id="CASE_REL_SEQ" name="caseRelSeq" value="0" /> <!-- 없음 -->
	<input type="hidden" id="CASE_TIME_E" name="caseTimeE" value="" /> <!-- 사용안함 -->
	<input type="hidden" id="CASE_VISIT" name="caseVisit" value="2" /> <!-- DECODE 사용해서 넣는 값 -->
	...
	<div class="view">
		...
		<div id="tabs-3">
			...
			<table class="table03">
				...
				<tbody>
					<tr>
						<th scope="row"><label for="caseType"><span class="essentialInputField req"><span>필수입력</span></span>피해구분1</label></th>
						<td>
							<select name="caseType" id="caseType" title="피해구분1" style="width: 90px;">
								<script>
									getCmmnCd.select('CSAB', 'caseType', '${result.caseType}');
								</script>
							</select>
						</td>
						<th scope="row"><label for="caseTypeGrp"><span class="essentialInputField req"><span>필수입력</span></span>피해구분2</label></th>
						<td>
							<select name="caseTypeGrp" id="caseTypeGrp" title="피해구분2" style="width: 120px">
								<script>
									getCmmnCd.select('CSACG', 'caseTypeGrp', '${result.caseTypeGrp}', 'N');
								</script>
							</select>
							<select name="caseTypeSub" id="caseTypeSub" title="피해구분2 서브" style="width: 140px">
								<script>
									getCmmnCd.select('CSAC', 'caseTypeSub', '${result.caseTypeSub}');
								</script>
							</select>
						</td>
						<td>기타선택시내용:
							<input type="text" name="caseTypeMemo" value="${result.caseTypeMemo}" style="width: 80px;">
						</td>
					</tr>
					<tr>
						<th scope="row"><label for="dmgStateYn"><span class="essentialInputField req"><span>필수입력</span></span>피해시 특이사항</label></th>
						<td colspan="4">
							<label>
								<input name="dmgStateYn" id="dmgStateYn" type="radio" value="N" <c:if test="${result.dmgStateYn eq 'N' }">checked="checked"</c:if> />없음
							</label>
							<label>
								<input name="dmgStateYn" type="radio" value="Y" <c:if test="${result.dmgStateYn eq 'Y' }">checked="checked"</c:if> />있음
							</label>
							<!-- 있음 선택 시 -->
							<div id="divDMG_STATE" style="display: none;">
								<script>
									getCmmnCd.checkBox('CSDG', 'divDMG_STATE', 'dmgState', '${result.dmgState}');
								</script>
							</div>
						</td>
					</tr>
...
<%--						<input type="text" name="dmgTime" id="dmgTime" value="<fmt:formatDate value="${result.dmgTime}" pattern="yyyy-MM-dd"/>"  class="date"> --%>
							<input type="text" name="dmgTime" id="dmgTime" value="${result.dmgTime}" class="formatDate">
							<input type="text" maxlength="20" name="dmgTimeMemo" value="${result.dmgTimeMemo}" style="width: 500px;">
						</td>
					</tr>
					<tr>
						<th>세부내용
						...
						<td colspan="4">
							<textarea name="caseMemo" id="caseMemo" style="height: 100px; width: 100%;">${resultMemo.caseMemo}</textarea>
						</td>
					</tr>
				</tbody>
			</table>
