...
<c:set var="action" value="${empty result ? '/bos/siteManage/visual/insert.do' : '/bos/siteManage/visual/update.do'}" />
<form id="fm" name="fm" method="post" action="${action}" enctype="multipart/form-data">
	...
	<c:if test="${not empty result}">
		<input type="hidden" name="visualNo" value="${param.visualNo}" />
	</c:if>
...
<table>
	...
	<tbody>
		<tr>
			<th scope="row"><span class="req"><span class="sr-only">필수입력</span></span>웹 비주얼 이미지</th>
			<td>
		...
		<tr>
			<th scope="row"><span class="req"><span class="sr-only">필수입력</span></span><label for="visualNm">제목 </label></th>
			<td>
				<input type="text" id="visualNm" name="visualNm" value="${result.visualNm}" style="width:100%" />
			</td>
		</tr>
		<tr>
			<th scope="row"><span class="req"><span class="sr-only">필수입력</span></span><label for="visualUrlad">링크 URL </label></th>
			<td><input type="text" id="visualUrlad" name="visualUrlad" value="${result.visualUrlad}" style="width:100%" />
				<p class="help">
...
<div class="btnSet">
	...
	<c:otherwise>
		...
		<a class="b-del"  href="/bos/siteManage/visual/delete.do?visualNo=${result.visualNo}&menuNo=${param.menuNo}&pSiteId=${param.pSiteId}" onclick="return confirm('정말로 삭제하시겠습니까?');"><span>삭제</span></a>
	</c:otherwise>
	</c:choose>
		<a class="b-cancel" href="/bos/siteManage/visual/list.do?menuNo=${param.menuNo}&pSiteId=${param.pSiteId}"><span>취소</span></a>
