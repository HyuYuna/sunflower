...
function selectMenu(node) {
	...
	//console.debug($("#guideMenuTable tr",opener.document));
	//console.debug($("input[name=pMenuNo]",opener.document));

	opener.fnSetMenuLink(node.id, node.data.allMenuCours);
	//$("input[name=pMenuNo]",opener.document).eq().val(node.id);
	//$("input[name=menuNm]",opener.document).eq(${param.idx}).val(node.text);
	//$("input[name=menuLink]",opener.document).eq(${param.idx}).val(node.data.allMenuCours);
	//console.debug(node.id + "|" + node.text + "|" + node.data.allMenuCours);
	self.close();
}
				...
				$.ajax({ url: t_url, dataType: "json"}).done(function (data) {
					...
					$.each(data,function(key, value) {
						if (value.useAt == "Y") {
							...
							var item = {'id' : value.menuNo, 'text' : value.menuNm, 'children' : children, 'icon' : value.icon};
							var addData = value;
							item.data = addData;
							jsonList.push(item);
						}
