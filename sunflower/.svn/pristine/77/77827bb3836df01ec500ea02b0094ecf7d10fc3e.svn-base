...
<script>
	function fnModify(frm) {
		...
		frm.action = "/${paramVO.siteId}/${paramVO.groupId}/${paramVO.programId}/update.do?useAt=Y";
		frm.target = '_self' ;
		frm.submit();
	}
	...
	function fnPreView(siteId){
		...
		var cont = document.modifyForm.colctCn.value;
...
<form name="modifyForm" enctype="multipart/form-data" action="/${paramVO.siteId}/${paramVO.groupId}/${paramVO.programId}/update.do" method="post">
	<input type="hidden" name="menuNo" id="menuNo" value="${paramVO.menuNo}"/>
	<input type="hidden" name="colctSe" id="colctSe" value="${paramVO.colctSe}"/>
	<input type="hidden" name="pSiteId" id="pSiteId" value="${empty result.pSiteId ? param.pSiteId : result.pSiteId}"/>
			...
			<tbody>
				<tr>
					<c:if test="${param.gubun ne 'Y'}">
						...
						<td>Ver.${empty result ? '최초' : result.colctMnno}</td>
					</c:if>
				</tr>
				<tr>
					<th scope="row">파일 경로</th>
					<td>${colctPath}</td>
				</tr>
				<tr>
					<th scope="row">제목</th>
					<td><input type="text" id="colctSj" name="colctSj" value="${result.colctSj}" style="width:100%" /></td>
				</tr>
				<tr>
					<th scope="row"><span class="req"><span>필수입력</span></span>내용</th>
					<td>
						<textarea rows="50" cols="150" id="colctCn" name="colctCn" style="height:200px" title="콘텐츠">${result.colctCn }</textarea>
...
<%-- 			<div class="fl">
			...
			<a class="b-del" href="/${paramVO.siteId}/${paramVO.groupId}/${paramVO.programId}/delete.do?menuNo=${result.menuNo}&colctMnno=${result.colctMnno}&pSiteId=${param.pSiteId}&colctSe=${param.colctSe}&viewType=${param.viewType}" onclick="return confirm('정말로 삭제하시겠습니까?')"><span>삭제</span></a>
...
<%-- 			<c:if test="${param.viewType ne 'BODY' }">
				<a class="b-cancel" href="/${paramVO.siteId}/${paramVO.groupId}/${paramVO.programId}/list.do?menuNo=${param.menuNo}&pSiteId=${param.pSiteId}&colctSe=${param.colctSe}"><span>취소</span></a>
			</c:if> --%>
		</div>
		...
		<div>
			<table class="table table-striped table-hover">
				<caption>History 목록</caption>
				<thead>
					<tr>
						<th style="width:15%" scope="col" class="fir">버전번호</th>
						<th style="width:15%" scope="col">제목</th>
				...
				<tbody>
					<c:forEach var="result2" items="${resultList}" varStatus="status">
						<tr ${(result2.colctMnno eq param.colctMnno) or (empty param.colctMnno and status.first) ? 'style="color:#f0f;"' : ''}>
							<td>Ver.${result2.colctMnno}</td>
							<td>
								<strong>
									<a href="forUpdate.do?colctSe=${result2.colctSe}&pSiteId=${param.pSiteId}&colctMnno=${result2.colctMnno}&menuNo=${param.menuNo}&viewType=${param.viewType}">${result2.colctSj}</a>
								</strong>
							</td>
							<td>${result2.useAt eq 'Y' ? '<font color="ff6600"><b>사용</b></font>' : '미사용'}</td>
							<td>
								<c:if test="${result2.useAt ne 'Y' }">
									<a class="btn btn-xs btn-info" href="updateUseAt.do?colctSe=${result2.colctSe}&pSiteId=${param.pSiteId}&colctMnno=${result2.colctMnno}&menuNo=${param.menuNo}&viewType=${param.viewType}">사용</a>
								</c:if>
							</td>
...
<form name="priviewForm" enctype="multipart/form-data" action="" method="post">
	...
	<input type="hidden" name="menuNm" value="${result.colctSj}" />
	<input type="hidden" name="menuNo" value="300067"/>
</form>
