...
<script>
	function join() {
		...
		$.post(
			"/ucms/qestnar/userAnswer/insert.json",
			data,
			function(data) {
				var resultCode = data.resultCode;
				if(resultCode == "success"){
					alert("정상처리했습니다.");
					location.href = "./list.do?menuNo=${param.menuNo}";
				}else {
					alert("등록에 실패하였습니다.");
				}
			},"json"
	...
	<div class="btnSet">
		<a href="/ucms/qestnar/qustnr/list.do?${pageQueryString}" class="b-list">목록</a>
	</div>
...
<form id="board" name="board" method="post" action="${action}">
	...
	<div class="btnSet">
		...
		<c:if test="${result.resultOthbcAt eq 'Y'}">
			<a href="/ucms/qestnar/answer/view.do?qustnrSn=${param.qustnrSn}&menuNo=${param.menuNo}" class="b-">결과보기</a>
		</c:if>
	</div>
</form>