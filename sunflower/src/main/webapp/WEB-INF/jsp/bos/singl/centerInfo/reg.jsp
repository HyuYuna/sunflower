...
<script>
	function fnModify(frm) {
		...
		frm.action = "/bos/singl/contents/update.do?useAt=Y";
		frm.target = '_self' ;
		frm.submit();
	}
	...
	function comparePop(sMenuNo) {
   		window.open("/bos/singl/contents/comparePop.do?viewType=BODY&sMenuNo="+sMenuNo,"comparePop","scrollbars=yes,width=1400,height=800");
   		return;
	}

</script>

<form name="modifyForm" enctype="multipart/form-data" action="/bos/singl/contents/update.do" method="post">
	...
	<c:if test="${param.gubun ne 'Y'}">
		<div class="btnSet">
			...
			<a class="b-del" href="/bos/singl/contents/delete.do?sUseAt=${paramVO.sUseAt}&useAt=${result.useAt}&sMenuNo=${result.menuNo}&cntntsMnno=${result.cntntsMnno}&menuNo=${param.menuNo}&sSiteId=${param.sSiteId}&viewType=${param.viewType}" onclick="return confirm('정말로 삭제하시겠습니까?')"><span>삭제</span></a>
				...
				<tbody>
					<c:forEach var="result2" items="${resultList}" varStatus="status">
						<tr>
							...
							<td>
								<strong>
									<a href="forUpdate.do?sMenuNo=${result2.menuNo}&sSiteId=${param.sSiteId}&cntntsMnno=${result2.cntntsMnno}&menuNo=${param.menuNo}&viewType=${param.viewType}">${empty result2.relateMenuNmList ? result2.menuNm : fn:replace(result2.relateMenuNmList,'|',' > ')}</a>
								</strong>
							</td>
							<td>${result2.useAt eq 'Y' ? '<font color="ff6600"><b>사용</b></font>' : '미사용'}</td>
							<td>
								<c:if test="${result2.useAt ne 'Y' }">
									<a class="btn btn-xs btn-info" href="/bos/singl/contents/updateUseAt.do?menuNo=${param.menuNo}&viewType=${param.viewType}&sSiteId=${param.sSiteId}&sMenuNo=${result2.menuNo}&cntntsMnno=${result2.cntntsMnno}">사용</a>
								</c:if>
							</td>
