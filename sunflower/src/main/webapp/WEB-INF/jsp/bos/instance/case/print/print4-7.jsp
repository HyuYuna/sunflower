...
<div class="c_both"></div>
    ...
    <h4>현재 이 사례의 사례관리자는 '${result.caseManager }' 입니다.</h4>

    <h4>[사례보관함] 여부 : 
    <c:if test="${result.caseFinish eq 'Y'}">사례보관함으로 이동된 사례</c:if>
    <c:if test="${result.caseFinish eq 'N'}">진행중인 사례</c:if>
    </h4>

    <h4>[자주보기사례] 여부 : 
    <c:if test="${resultBookCnt > 0}" >자주보기 사례</c:if>
    <c:if test="${resultBookCnt <= 0}" >아님</c:if>
    </h4>
        ...
        <table cellpadding="0" cellspacing="0" border="0" class="table02">
            ...
            <tbody>
                ...
                <c:forEach var="result" items="${resultCaseHistory}" varStatus="status">
                <tr>
                    <td style="text-align:center;">${status.count}</td>
                    <td style="text-align:center;">${result.chDate}</td>
                    <td style="text-align:center;">${result.chUserNm}</td>
                    <td style="text-align:center;">${result.chGroup}</td>
                    <td style="text-align:center;">${result.chAction}</td>
                    <td style="text-align:left;">${result.chMemo}</td>
                </tr>
                </c:forEach>
            </tbody>
        </table>
