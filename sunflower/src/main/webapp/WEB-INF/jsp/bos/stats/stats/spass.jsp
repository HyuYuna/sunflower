...
<script>
	function filePopZip(){
	    ...
	    var params = "?shDataNm=통계(서비스)&viewType=BODY";
	...
	function fn_export(){
	    ...
	    $(this).attr('download', '통계_서비스.xls').attr('href', uri).attr("target", "_blank");
	}
	function numComma(n) {
		if(n!=null){
	...
	function getResultData() {
		...
		$.ajax({
            type:"post",
            url: "/bos/stats/stats/sPassStats.json",
            dataType: "json",
			...
			success : function(data){
            	...
            	var sPassStats = data.sPassStats;
            	var sPassStatd = data.sPassStatd;
				var table = "";
            	...
            	if(codeResult.length >= 0){
            		...
            		var preGroupCode = "0";
            		var cCode = "";
            		$.each(sPassStats, function(i, entry) {
            			if(i == 0){
            				if(sexYN == 'Y'){
            			...
            			if(data.totYN != 'Y'){
            				if(sexYN == 'Y'){
            					...
            					//전체 합계값 출력
            					for(var t=6; t<data.codeCount2; t++){
            						$.each(sPassStats, function(index, stats) {
            						...
            						if(dsaYN == "Y"){
            							$.each(sPassStatd, function(index, statd) {
            					...
            					if(entry["victimSex"] == "M"){
            						for(var t=6; t<data.codeCount2; t++){
            							...
            							if(dsaYN == "Y"){
            								...
            								sumDrray[t] = sPassStatd[i]["fieldname" + data.codename[t]];
            							}
            						}
            						...
            						for(var t=6; t<data.codeCount2; t++){
            							...
            							if(dsaYN == "Y"){
            								...
            								sumDrray[t] = sumDrray[t] + sPassStatd[i]["fieldname" + data.codename[t]];
            							}
            						}
...
/*
    ...
    function fn_centerGubunCodeSubChange(){
					...
					<tr>
						...
						<td colspan=3>
							<select name="src_CNTR_GBN" id="src_CNTR_GBN" title="센터 구분" onchange="getCmmnCd.subSelect('CM05','src_CNTR_COD',this.value , '', 'N')">
								<script>
									getCmmnCd.selectStats('CM04', 'src_CNTR_GBN', '${paramVO.src_CNTR_GBN}', 'N', '전체 선택');
								</script>
							</select>
							&nbsp;
							<select name="src_CNTR_COD" id="src_CNTR_COD" title="센터 구분 서브">
								<script>
									getCmmnCd.subSelect('CM05', 'src_CNTR_COD', '${paramVO.src_CNTR_GBN}', '${paramVO.src_CNTR_COD}', 'N', '전체 선택')
								</script>
							</select> 
        ...
        function setCookies4CaseGubun() {
            ...
            $.cookie('srcCookieGubun05',  setCheck03,  { expires: 7, path: '/' });
            $.cookie('srcCookieGubun06',  setCheck03,  { expires: 7, path: '/' });
        ...
        */
			...
			<tr class="trExtend" style="height: 40px;">
				...
				<td colspan=3>
					...
					<select id='SUB1_DATA' name='SUB1_DATA' style='width: 40%;' ref='PSA2'>
						<script>
							getCmmnCd.selectStats('PSA2', 'SUB1_DATA', '${paramVO.SUB1_DATA}', '', '', 'Y');
						</script>
					</select>&nbsp;
				</td>
			</tr>
        ...
        fnc_SUB_PASS_TYPE();
			...
			<tr style="height: 40px;">
				...
				<td colspan=3>
					<span style="display: inline-block; width: 80px;"><input type="checkbox" name="dsaYN" id='dsaYN' value="Y">
						장애
					</span> 
					<span style="display: inline-block; width: 100px;"><input type="checkbox" name="sexYN" id='sexYN' value="Y">
						성별 구분
        ...
        */
			...
			<tr class="trExtend" style="height: 40px;">
				<td style="text-align: center;">내담자 구분</td>
				<td colspan=2>
					<select id='PASS_REL_TYPE' name='PASS_REL_TYPE' style='width: 100px;' ref='PSRY'>
						<script>
							getCmmnCd.selectStats('PSRY', 'PASS_REL_TYPE', '${paramVO.PASS_REL_TYPE}', 'N', '전체');
						</script>
					</select>
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
					내담자와 피해자와의 관계 : 
					<select id='USER_REL_GRP' name='USER_REL_GRP' style='width: 100px;' ref='CS25G'>
						<script>
							getCmmnCd.selectStats('CS25G', 'USER_REL_GRP', '${paramVO.USER_REL_GRP}', 'N', '전체');
						</script>
					</select>&nbsp;
					<select id='USER_REL' name='USER_REL' style='width: 150px;' ref='CS25'>
						<script>
							getCmmnCd.selectStats('CS25', 'USER_REL', '${paramVO.USER_REL}', 'N', '전체');
						</script>
					</select>
				</td>
				<td>
					<span style="display: inline-block; width: 150px;"><input type="checkbox" name="VISIT_JIWON_CHECK" value="N"> 내방 지원 여부</span>
				</td>
			</tr>
			<tr>
				<td style="text-align: center;">지원방법</td>
				<td colspan=3>
					<select id='PASS_WAY' name='PASS_WAY' style='width: 100px;' ref='PSRY'>
						<script>
							getCmmnCd.selectStats('CS21', 'PASS_WAY', '${paramVO.PASS_WAY}', 'N', '전체');
						</script>
					</select>
				</td>
			</tr>
        ...
        //페이지 로딩시 기본 정보값 로드
        var $USER_REL_OPTION  = $('#USER_REL>option').clone();

        //피해자와의 관계 하위 선택
        $("#formSearch select[name='USER_REL_GRP']").change(function() {
            fnc_SUB_USER_REL();
        });
        ...
        function fnc_SUB_USER_REL() {
            var selectOptionVal = $("#formSearch select[name='USER_REL_GRP']").val();
            var selectOptionSub = $("#formSearch select[name='USER_REL_GRP']").find('option:selected').attr('data-subcode');
            if ( jQuery.type( selectOptionSub )=="undefined" || selectOptionSub=="") {
                $("#formSearch select[name='USER_REL']").prop('disabled', true).hide();
            } else {

                $('#USER_REL').html($USER_REL_OPTION);

                $("#formSearch select[name='USER_REL']").prop('disabled', false).show().find("option").filter(function(index) {
                    var dataGrpcde = $(this).attr("data-grpcode");
                    if (typeof dataGrpcde != "undefined") {
                        if ( $(this).attr("data-grpcode").search(selectOptionVal) == 0 ){ return false;} else { return true; }
                    }
            ...
            if(selectOptionVal == '10' && $("#formSearch input[name='USER_NAME']").val()=="") {
                $("#formSearch input[name='USER_NAME']").val("");
            }
        }
			</script>
