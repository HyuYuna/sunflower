    ...
    $("#pageSelect").change(function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $("#frm2").attr("action", "/bos/member/user/list_login.do");
                $("#frm2").submit();
            }
        });
    });
...
<form id="frm2" name="frm2" method="post" action="/bos/member/user/list_login.do" class="pure-form">
    ...
    <fieldset>
        <div class="sh">
            ...
            <input id="sdate1" name="sdate1" class="formatDate" title="시작일" placeholder="접속 시작일" value="${paramVO.sdate1}" type="text" readonly="readonly"> ~
            <input id="edate1" name="edate1" class="formatDate" title="종료일" placeholder="접속 종료일" value="${paramVO.edate1}" type="text" readonly="readonly">
            ...
            <a href="/bos/member/user/list_login.do?menuNo=${param.menuNo}" class="pure-button b-reset">초기화</a>
        </div>
    </fieldset>
    <!-- 게시판 게시물검색 end -->
        ...
        <table id="jqlblList" style="width:100%;" class="table table-striped table-hover">
            ...
            <tbody>
                <c:forEach var="result" items="${resultList}" varStatus="status">
                    <tr>
                        <td>${result.mlhusercenter}</td>  
                        <td>${result.mlhuserid}</td>
                        <td>${result.mlhusername}</td>
                        <td>${result.mlhuserip}</td>
                        <td><fmt:formatDate value="${result.mlhlogindatetime }" pattern="yyyy-MM-dd HH:mm:ss"/></td>
                        <!-- <td><fmt:formatDate var="loginDate" value="${result.mlhLoginDateTime}" pattern="YYYY-MM-dd"/>${loginDate}</td> -->  
                    </tr>
...
<div class="btnSet">
        <a class="pure-button b-down" href="javascript:filePopZip('downloadExcel6','${param.searchCnd1}','${param.searchCnd2}','${param.searchCnd3}','${param.searchCnd4}','${param.searchCnd5}','${param.searchWrd}','${param.orderName}','${param.orderBdate}','${param.sdate1}','${param.edate1}')">엑셀다운</a>
</div>
