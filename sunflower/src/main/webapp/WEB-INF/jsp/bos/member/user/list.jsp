    ...
    $("#pageSelect").change(function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $("#frm2").attr("action", "/bos/member/user/list.do");
                $("#frm2").submit();
            }
        });
    });
...
<form id="frm2" name="frm2" method="post" action="/bos/member/user/list.do" onsubmit="return goSearch(this);" class="pure-form">
    ...
    <fieldset>
        <div class="sh">
            ...
            <select name="searchCnd3" id="searchCnd3" title="권한" >
                <script>
                    getCmmnCd.select('USLV','searchCnd3','${paramVO.searchCnd3}', 'N')
                </script>
            </select>
            <select name="searchCnd4" id="searchCnd4" title="직군" >
                <script>
                    getCmmnCd.select('CDOP','searchCnd4','${paramVO.searchCnd4}', 'N')
                </script>
            </select>
            ...
            <input type="text" name="searchWrd2" id="searchWrd2" value="${paramVO.searchWrd2}" class="" placeholder="사용자아이디" title="사용자아이디" />
        ...
        <table id="jqlblList" style="width:100%;" class="table table-striped table-hover">
            ...
            <tbody>
                <c:forEach var="result" items="${resultList}" varStatus="status">
                    <tr style="cursor: pointer;" id="trCase" ondblclick="javascript:goUserView('${result.userId}');">
                        <td>${result.userGroupName}</td>
                        ...
                        <td>
                            <fmt:parseDate var="userBCdate" value="${result.userBCdate }" pattern="yyyy-MM-dd"/>
                        	<fmt:formatDate value="${userBCdate}" pattern="yyyy-MM-dd"/>
                        </td>
                        <td>
                        	<c:choose>
                        		<c:when test="${result.userState eq 'Y' }">
                        			재직
                        		</c:when>
                        		<c:when test="${result.userState eq 'S' }">
                        			휴직
                        		</c:when>
                        		<c:when test="${result.userState eq 'N' }">
                        			퇴직
                        		</c:when>
                        	</c:choose>
                        </td>
                        <td><c:choose>
                                <c:when test="${empty result.lastLoginDate}">
                                    -
                                </c:when>
                                <c:otherwise>
...
<div class="btnSet">
        ...
        <a class="pure-button b-down" href="javascript:filePopZip('downloadExcel','${param.searchCnd1}','${param.searchCnd2}','${param.searchCnd3}','${param.searchCnd4}','${param.searchCnd5}','${param.searchWrd}','${param.orderName}','${param.orderBdate}','${param.sdate1}','${param.edate1}','${userVO.centerCode}','${userVO.authorCd}')">엑셀다운 </a>
</div>
<%--\\
<div class="btnSet">
