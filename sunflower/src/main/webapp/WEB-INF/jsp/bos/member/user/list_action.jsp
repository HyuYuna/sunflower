...
<script>
function filePopZip(now,searchCnd1,searchCnd2,searchCnd3,searchCnd4,searchCnd5,searchWrd,orderName,orderBdate,sdate1,edate1,chGroup,chAction){
    var now = now;
    ...
    var params = "?viewType=BODY&urlType=zip&searchCnd1="+searchCnd1+"&searchCnd2="+searchCnd2
            ...
            +"&orderName="+orderName+"&orderBdate="+orderBdate+"&sdate1="+sdate1+"&edate1="+edate1+"&now="+now+"&chGroup="+chGroup+"&chAction="+chAction+"&viewType=CONTBODY";
    ...
    $("#pageSelect").change(function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $("#frm2").attr("action", "/bos/member/user/list_action.do");
                $("#frm2").submit();
            }
        });
    });
...
<form id="frm2" name="frm2" method="post" action="/bos/member/user/list_action.do" onsubmit="return goSearch(this);" class="pure-form">
    ...
    <fieldset>
        <div class="sh">
            ...
            <select name="chGroup" id="chGroup" title="작업그룹">
                <option value="">선택</option>
                <option value="CASE"  <c:if test="${'CASE' eq paramVO.chGroup}">selected="selected"</c:if>>사례기본정보</option>
                <option value="PASS"  <c:if test="${'PASS' eq paramVO.chGroup}">selected="selected"</c:if>>지원서비스</option>
                <option value="SERV"  <c:if test="${'SERV' eq paramVO.chGroup}">selected="selected"</c:if>>사례부가정보</option>
                <option value="ATTKR" <c:if test="${'ATTKR' eq paramVO.chGroup}">selected="selected"</c:if>>가해자정보</option>
                <option value="FILE"  <c:if test="${'FILE' eq paramVO.chGroup}">selected="selected"</c:if>>첨부파일</option>
                <option value="STATS" <c:if test="${'STATS' eq paramVO.chGroup}">selected="selected"</c:if>>사례통계</option>
                <option value="TEL"   <c:if test="${'TEL' eq paramVO.chGroup}">selected="selected"</c:if>>연락처</option>
            </select>
            <select name="chAction" id="chAction" title="작업내역">
                <option value="">선택</option>
                <option value="V" <c:if test="${'V' eq paramVO.chAction}">selected="selected"</c:if>>조회</option>
                <option value="I" <c:if test="${'I' eq paramVO.chAction}">selected="selected"</c:if>>입력</option>
                <option value="U" <c:if test="${'U' eq paramVO.chAction}">selected="selected"</c:if>>수정</option>
                <option value="D" <c:if test="${'D' eq paramVO.chAction}">selected="selected"</c:if>>삭제</option>
                <option value="P" <c:if test="${'P' eq paramVO.chAction}">selected="selected"</c:if>>출력</option>
                <option value="S" <c:if test="${'S' eq paramVO.chAction}">selected="selected"</c:if>>검색</option>
            </select>
            ...
            <tbody>
                <c:forEach var="result" items="${resultList}" varStatus="status">
                    <tr>
                        ...
                        <td>${result.chUserId}</td>
                        <td>${result.chUserName}</td>
                        <td><fmt:formatDate value="${result.chDate }" pattern="yyyy-MM-dd HH:mm:ss"/></td>
                        <td>사례(${result.caseNumber}) ${result.chGroupName} ${result.chActionName}</td>
                    </tr>
...
<div class="btnSet">
        <a class="pure-button b-down" href="javascript:filePopZip('downloadExcel3','${param.searchCnd1}','${param.searchCnd2}','${param.searchCnd3}','${param.searchCnd4}','${param.searchCnd5}','${param.searchWrd}','${param.orderName}','${param.orderBdate}','${param.sdate1}','${param.edate1}','${param.chGroup}','${param.chAction}')">엑셀다운</a>
</div>
